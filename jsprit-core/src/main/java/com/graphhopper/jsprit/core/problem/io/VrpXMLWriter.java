...
//				xmlConfig.setProperty(solutionPath + "(" + counter + ").routes.route(" + routeCounter + ").cost", route.getCost());
    ...
    private void writeServices(XMLConf xmlConfig, List<Job> jobs) {
        ...
        for (Job j : jobs) {
            ...
            xmlConfig.setProperty(shipmentPathString + "(" + counter + ").setupDuration", service.getSetupDuration());
...
//			xmlConfig.setProperty(shipmentPathString + "("+counter+")[@type]", service.getType());
            ...
            xmlConfig.setProperty(shipmentPathString + "(" + counter + ").pickup.setupDuration", shipment.getPickupSetupTime());
            ...
            xmlConfig.setProperty(shipmentPathString + "(" + counter + ").delivery.setupDuration", shipment.getDeliverySetupTime());
        	index = 0;
    ...
    private void writeVehiclesAndTheirTypes(XMLConfiguration xmlConfig) {
        ...
        for (Vehicle vehicle : vrp.getVehicles()) {
            ...
            xmlConfig.setProperty(vehiclePathString + "(" + counter + ").setupCoef", vehicle.getCoefSetupTime());
        ...
        for (VehicleType type : vrp.getTypes()) {
            ...
            xmlConfig.setProperty(typePathString + "(" + typeCounter + ").costs.setup", type.getVehicleCostParams().perSetupTimeUnit);
            typeCounter++;
        }
