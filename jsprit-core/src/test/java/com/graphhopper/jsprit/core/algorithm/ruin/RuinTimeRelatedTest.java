 ...
 */
@DisplayName("Ruin Time Related Test")
class RuinTimeRelatedTest {

    @Test
    @DisplayName("It Should Ruin Two Obvious Clusters")
    void itShouldRuinTwoObviousClusters() {
        ...
        VehicleRoutingProblem vrp = VehicleRoutingProblem.Builder.newInstance().addJob(s1).addJob(s2).addJob(s6).addJob(s7).addJob(s0).addJob(s3).addJob(s4).addJob(s5).addVehicle(v).build();
        ...
        VehicleRoute vr2 = VehicleRoute.Builder.newInstance(v).addService(s6).addService(s7).addService(s4).addService(s5).setJobActivityFactory(vrp.getJobActivityFactory()).build();
        time = 0;
