...
@DisplayName("Position Based Regret Insertion Test")
class PositionBasedRegretInsertionTest {
    ...
    void shouldInsertAllServicesIntoSingleRoute() {
        ...
        PositionBasedRegretInsertion insertion = new PositionBasedRegretInsertion(calculator, vrp);
    ...
    void shouldInsertAllShipmentsIntoSingleRoute() {
        ...
        assertEquals(1, routes.size());
        // 2 shipments = 4 activities (2 pickups + 2 deliveries)
        assertEquals(4, routes.iterator().next().getActivities().size());
    }

    @Test
    @DisplayName("Should assign services to separate routes with multiple vehicles")
    void shouldAssignServicesToSeparateRoutesWithMultipleVehicles() {
        // Services at opposite ends, with vehicles near each
        ...
        PositionBasedRegretInsertion insertion = new PositionBasedRegretInsertion(calculator, vrp);
        for (com.graphhopper.jsprit.core.algorithm.recreate.listener.InsertionListener l : iListeners) {
        ...
        // With proper cost calculation, services should be assigned to nearest vehicles
    ...
    @Test
    @DisplayName("Should work with k=3 positions")
    void shouldWorkWithK3Positions() {
        Service s1 = Service.Builder.newInstance("s1").setLocation(Location.newInstance(0, 10)).build();
        Service s2 = Service.Builder.newInstance("s2").setLocation(Location.newInstance(0, 5)).build();
    ...
    void shouldBeCreatableViaInsertionStrategyBuilder() {
        ...
        InsertionStrategy strategy = new InsertionStrategyBuilder(vrp, fm, stateManager, constraintManager)
                .setInsertionStrategy(InsertionStrategyBuilder.Strategy.POSITION_BASED_REGRET)
                .setPositionBasedRegretK(3)
                .build();

        assertNotNull(strategy);
        assertTrue(strategy instanceof PositionBasedRegretInsertion);
