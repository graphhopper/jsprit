...
public class LiLimReader {
    ...
    private void buildShipments() {
        ...
        for (Relation rel : relations) {
            ...
            Shipment s =
                    Shipment.Builder
                            .newInstance(counter.toString())
                            .addSizeDimension(0, demand)
                            .setPickupLocation(
                                    PickupLocation.Builder.newInstance()
                                            .setLocation(Location.Builder.newInstance().setCoordinate(customers.get(from).coord).build())
                                            .addTimeWindow(TimeWindow.newInstance(customers.get(from).start, customers.get(from).end)).build()
                            )
                            .setPickupServiceTime(customers.get(from).serviceTime)
                            .setDeliveryLocation(Location.Builder.newInstance().setCoordinate(customers.get(to).coord).build())
