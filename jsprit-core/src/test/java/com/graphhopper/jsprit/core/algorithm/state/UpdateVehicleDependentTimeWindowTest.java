 ...
 */
@DisplayName("Update Vehicle Dependent Time Window Test")
class UpdateVehicleDependentTimeWindowTest {
    ...
    @Test
    @DisplayName("When Switch Is Not Allowed _ it Should Cal Only States Of Current Vehicle")
    void whenSwitchIsNotAllowed_itShouldCalOnlyStatesOfCurrentVehicle() {
        stateManager = new StateManager(vrp);
        UpdateVehicleDependentPracticalTimeWindows updater = new UpdateVehicleDependentPracticalTimeWindows(stateManager, routingCosts, activityCosts);
        stateManager.addStateUpdater(updater);
        stateManager.informInsertionStarts(Arrays.asList(route), Collections.<Job>emptyList());
    ...
    @Test
    @DisplayName("Tw Update Should Work With Multiple T Ws")
    void twUpdateShouldWorkWithMultipleTWs() {
        //
    ...
    @Test
    @DisplayName("Update Of Open Routes Should Be Done Correctly")
    void updateOfOpenRoutesShouldBeDoneCorrectly() {
        VehicleImpl vehicle = VehicleImpl.Builder.newInstance("v").setReturnToDepot(false).setStartLocation(Location.Builder.newInstance().setCoordinate(Coordinate.newInstance(0, 0)).build()).setLatestArrival(51).build();
        ...
        VehicleRoutingProblem vrp = VehicleRoutingProblem.Builder.newInstance().addJob(service).addVehicle(vehicle).setFleetSize(VehicleRoutingProblem.FleetSize.FINITE).build();
